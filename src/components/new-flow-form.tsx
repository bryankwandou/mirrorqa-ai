...
export function NewFlowForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const selected = personas.filter((item) => form.get(item.key) === "on");
    if (!selected.length) {
      setError("Select at least one customer profile.");
      return;
    }
    setBusy(true);
    try {
      ...
      for (const persona of selected) {
        const response = await fetch("/api/browser/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ persona: persona.key, goal: flow.goal, maxSteps: Number(flow.maxSteps) }) });
        const result = await response.json();
        if (!response.ok) throw new Error(`${persona.name}: ${result.error || "The browser run could not start."}`);
        runs.push({ persona, result });
      }
      sessionStorage.setItem("mirrorqa:last-run", JSON.stringify({ flow, runs }));
  ...
  return <form className="flow-form" onSubmit={submit}>
    ...
    <section className="form-section"><span className="form-index">02</span><div><h2>Choose perspectives</h2><p>Every selected profile gets a separate Chromium context and model trace.</p></div><div className="persona-select">{personas.map((persona) => <label key={persona.key}><input name={persona.key} type="checkbox" defaultChecked /><span className={`persona-number ${persona.color}`}>{persona.mark}</span><span><b>{persona.name}</b><small>{persona.note}</small></span><Check size={15} /></label>)}</div></section>
    ...
    <div className="form-actions"><Link className="button ghost" href="/flows">Cancel</Link><button className="button primary" type="submit" disabled={busy}>{busy ? <LoaderCircle className="spin" size={15} /> : null}{busy ? "Running selected personas..." : "Register and run selected"}</button></div>
  </form>;
}
